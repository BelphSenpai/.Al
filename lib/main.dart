...
class _HomePageState extends State<HomePage> {
  ...
  int calculateTotalPoints() {
    List<int> valoresSinPilares = [];
    for (int i = 0; i < selectedValues.length; i++) {
      if (selectedCastells[i] != 'Pd' || i != selectedValues.length - 1) {
        valoresSinPilares.add(selectedValues[i]);
      }
    }

    valoresSinPilares.sort((a, b) => b.compareTo(a));
    int total = 0;
    if (valoresSinPilares.length >= 3) {
      total = valoresSinPilares.take(3).reduce((a, b) => a + b);
    }
    total += selectedValues.last;
    return total;
  }
